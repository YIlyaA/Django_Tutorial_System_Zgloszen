...
4. Więcej mozna zobaczyć tu: [dokumentacja Django](https://docs.djangoproject.com/en/6.0/ref/models/fields/#field-types)
...
1. nazwę użytkownika,
2. adres e-mail (moze być dowolny),
3. hasło.
...
def edytuj_zgloszenie(request, pk):
  # Pobieramy konkretne ogłoszenie na podstawie jego klucza głównego (pk).
  ...
  if request.method == "POST":
    ...
    # Przy wejściu metodą GET, wypełniamy formularz aktualnymi danymi ogłoszenia.
...
def usun_zgloszenie(request, pk):
  # Pobieramy konkretne ogłoszenie do usunięcia na podstawie ID (pk).
  # Jeśli ogłoszenie nie istnieje, użytkownik zobaczy błąd 404.
  zgloszenie = get_object_or_404(Zgloszenie, pk=pk)
...
Ponownie stwórz zgłosznie i zapisz:
